...
VulkanSwapchain::VulkanSwapchain(Junia::Window* window, VkSurfaceKHR surface, uint8_t maxInFlightFrames)
	...
	renderPass = new VulkanRenderPass(format, JMath::Vec4f(1.0f, 0.0f, 1.0f, 1.0f));
	Recreate();
...
void VulkanSwapchain::Recreate() {
	...
	std::shared_ptr<Junia::Shader> vertexShader = Junia::Shader::Create("Assets/Shaders/shader1_vert.spv", Junia::Shader::Type::VERTEX);
	std::shared_ptr<Junia::Shader> fragmentShader = Junia::Shader::Create("Assets/Shaders/shader1_frag.spv", Junia::Shader::Type::FRAGMENT);
	std::shared_ptr<Junia::Material> material = Junia::Material::Create(vertexShader, fragmentShader);
	graphicsPipeline = new VulkanGraphicsPipeline(renderPass->Get(), material);
}
...
void VulkanSwapchain::Draw() {
	...
	renderPass->Begin(currBuffer, framebuffers[imageIndex], extent);
	...
	commandPool->SubmitBuffer(currentFrame, vkDevice->GetGraphicsQueue(),
		&imageAvailableSemaphores[currentFrame], 1, waitStages, &renderFinishedSemaphores[currentFrame], 1, inFlightFences[currentFrame]);
